...
ia_sql = (f"""
        SELECT ia.state, ia.ZIPCODE, g.AVS_LATITUDE,g.AVS_LONGITUDE, 
        sum(ia.APPROVEDFORFEMAASSISTANCE) AS approved_assistances 
        FROM  disaster_data.dv.FEMA_IA ia
        LEFT JOIN disaster_data.dv.STATE_ZIPCODE_GEOMETRY g 
        ON ia.state=g.state AND ia.ZIPCODE =g.HOUSE_ZIP 
        WHERE ia.state='TX' AND DISASTERNUMBER =4798
            AND g.AVS_LATITUDE IS NOT NULL AND g.AVS_LONGITUDE IS NOT NULL 
        GROUP BY ia.state,ia.zipcode, g.AVS_LATITUDE, g.AVS_LONGITUDE
        HAVING approved_assistances >3000
...
runsql = (f"""
    SELECT dv.FEMA_DISASTER_NUMBER, dv.DISASTER_INDICATOR,dv.DISASTER_TYPE,dv.zipcode,
    ...
    GROUP BY dv.FEMA_DISASTER_NUMBER, dv.DISASTER_INDICATOR,dv.DISASTER_TYPE,dv.zipcode
    """)
...
for i in m_df.index:
    ...
    pop_txt = folium.Popup(f"""
                           zipcode:{m_df.ZIPCODE[i]},homes:{homes}""",
                           max_width=150,
    )
...
for k in ia_data.index:
    ...
    pop_txt = folium.Popup(f"""
                           Zipcode:{ia_data.ZIPCODE[k]}
                           """)
    if lat and long:
        marker = folium.CircleMarker([lat, long], popup=pop_txt, radius=1,color='blue', fill=True,fill_color='blue',fill_opacity=0.5).add_to(fg5)
